#!/usr/bin/env python3
"""
ZED Camera FHD (1080p @ 15Hz) H.264 (MP4) Recorder and ROS 2 Publisher Node.

This node captures FHD video from a USB-connected ZED camera at 15 FPS,
saves the left eye video to an H.264/MP4 file in real-time,
and simultaneously publishes the ROS 2 Image and CompressedImage topics.
"""
...
class ZedH264RecorderPublisher(Node):

    def __init__(self):
        super().__init__('zed_h264_recorder_publisher')
        ...
        self.get_logger().info(f'ZED H.264 Recorder & Publisher started on topic "{topic_name}" at {self.frame_rate} FPS')
    ...
    def timer_callback(self):
        ...
        if frame is not None:
            ...
            # 2. Write frame to MP4 file
            if self.writer is not None and self.writer.isOpened():
                self.writer.write(frame)

            # 3. Publish to ROS 2 topics
